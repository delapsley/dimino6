...
-record(state, {xcube_host, xcube_port}).
...
handle_call({send_x3c_cmd, Type, Args}, _From, State) ->
    ...
    error_logger:info_msg("handle_call:command xml_string == ~p~n", [Xml_string]),
    {ok, Sock} = gen_tcp:connect(State#state.xcube_host,
				 State#state.xcube_port,
				 ?TCP_OPTIONS),
    gen_tcp:send(Sock, Xml_string),
    {ok, Response} = gen_tcp:recv(Sock, 0, ?DEFAULT_BLOCK_SIZE),
    error_logger:info_msg("handle_call:response ~p~n",
			  [Response]),
    gen_tcp:close(Sock),
...
param_tuple_to_record(Keyvalpairs) ->
    #param{pname=proplists:get_value(pname, Keyvalpairs), 
